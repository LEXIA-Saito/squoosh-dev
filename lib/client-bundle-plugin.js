...
export function getDependencies(clientOutput, item) {
  ...
  for (const fileName of crawlDependencies) {
    ...
    // Guard against missing chunks (e.g., assets or externals)
    if (!chunk) continue;
...
export default function (inputOptions, outputOptions, resolveFileUrl) {
  ...
  return {
    ...
    resolveImportMeta(property, { moduleId, format }) {
      ...
      // Gracefully handle missing client entry (e.g., tree-shaken or asset-only)
      if (property.startsWith(entryPathPlaceholder)) {
        if (!clientEntry) return JSON.stringify("");
      ...
      const dependencies = clientEntry
        ? getDependencies(clientOutput, clientEntry)
        : [];

      if (property.startsWith(allSrcPlaceholder)) {
        // Collect chunks that have code; guard against missing entries
        const depEntries = dependencies
          .map((name) => clientOutput.find((item) => item && item.fileName === name))
          .filter(Boolean);
        const baseModules = clientEntry ? [clientEntry, ...depEntries] : depEntries;
        const allModules = baseModules.filter(
          (item) => item && Object.prototype.hasOwnProperty.call(item, 'code') && item.code,
        );

        const inlineDefines = [
          ...allModules.map((item) => {
            // Extra safety: ensure item and item.code are defined
            if (!item || !item.code) return '';
            return `self.nextDefineUri=location.origin+${resolveFileUrl(item)};${item.code}`;
    ...
    async generateBundle(options, bundle) {
      ...
      for (const clientEntry of clientOutput) {
        ...
        // Some entries may be assets without source or chunks with code; guard accordingly
        const hasCode = Object.prototype.hasOwnProperty.call(clientEntry, 'code');
        const hasSource = Object.prototype.hasOwnProperty.call(clientEntry, 'source');
        const content = hasCode ? clientEntry.code : hasSource ? clientEntry.source : undefined;
